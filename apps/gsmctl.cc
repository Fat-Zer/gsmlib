...
static void printInfo(InfoParameter ip)
{
  ...
  case OperatorInfo:
  {
    try {
      ...
      for (std::vector<gsmlib::OPInfo>::iterator i = opis.begin(); i != opis.end(); ++i)
      {
        ...
        std::cout << _("<OP> Operator Selection: ") << _("unsupported") << std::endl;
    }
    break;
  }
  ...
  case FacilityLockStateInfo:
  {
    try {
      ...
      for (std::vector<std::string>::iterator i = fclc.begin(); i != fclc.end(); ++i)
        ...
        std::cout << _("<FLSTAT> Facility Lock State Info: ") << _("unsupported") << std::endl;
    }
    break;
  }
  case FacilityLockCapabilityInfo:
  {
    try {
      ...
      std::cout << std::endl;
        ...
        std::cout << _("Facility Lock Capability Info: ") << _("unsupported") << std::endl;
    }
    break;
  }
  case PasswordInfo:
  {
    try {
      ...
      for (std::vector<gsmlib::PWInfo>::iterator i = pwi.begin(); i != pwi.end(); ++i)
      {
        ...
        std::cout << _("<PW> Change Password: ") << _("unsupported") << std::endl;
    }
    break;
  }
  case PINInfo:
  ...
  case CLIPInfo:
  {
    try {
      ...
      std::cout << _("<CLIP0> Calling Line Identification Presentation: ") << _("unsupported") << std::endl;
    }
    break;
  }
  case CallForwardingInfo:
  {
    try {
      for (int r = 0; r < 4; ++r)
      {
        ...
        std::cout << _("<FORW> Call Forwarding number and Conditions: ") << _("unsupported") << std::endl;
    }
    break;
  }
  case BatteryInfo:
